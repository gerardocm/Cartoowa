...
import time
from video_processing.video_to_frames import convert_to_frames
from video_processing.frames_to_video import convert_to_video
...
def predict_file(input_path, output_path):
    ...
    try:
        ...
        print(input_path, ": Invalid file extension.")
        return
...
if __name__ == "__main__":
    ...
    # parser.add_argument("-s", "--size", type=str, default="320,320")
    # parser.add_argument("-fps", "--frames", type=int, default=None)
    ...
    map_location='cpu'
    # Load weights
    netG.load_state_dict(torch.load(pretrained_dir, map_location=map_location))

    # Checks video directory
    temp_frames_path = "temp/frames"
    temp_output_path = "temp/output"
    if os.path.isfile(input_path):
        convert_to_frames(input_path, temp_frames_path, 30, (320,320))
    else:
        raise IOError("Fail loading the video")

    os.makedirs(temp_output_path, exist_ok=True)
    for file_name in tqdm(os.listdir(temp_frames_path), desc="Processing files"):
        file_path = os.path.join(temp_frames_path, file_name)
        output_file_path = os.path.join(temp_output_path, file_name)
        predict_file(file_path, output_file_path)

    convert_to_video(temp_output_path, output_path, 30)
